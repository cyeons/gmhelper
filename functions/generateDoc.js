...
exports.handler = async (event) => {
  ...
  let prompt = `
    ...
    - 본문: 첫 줄은 "${firstLine}"를 기반으로 공문 관례에 맞게 보완, 입력값에 "하고자합니다"가 포함되면 "~하고자 합니다"로 유지 또는 보완(하위 항목 없으면 단독 문장으로 끝냄), 안내 의도(예: "안내" 단어 포함, 하위 항목 있음)면 "~를 다음과 같이 안내합니다"로, 그 외는 입력값 의미에 따라 "~를 [동사]합니다"로 보완, "구입합니다" 등 직설적 표현 피하고 관례적 표현 우선, 불필요한 단어 추가 최소화, 하위 항목은 첫 줄 의도와 맥락적으로 맞는 경우에만 입력값을 토대로 공백 2칸 들여쓰기 후 "가. ", "나. " 등으로 항목화(예: "구입"이면 "구입 품목", "예산" 등, "안내"이면 "일시", "장소" 등), 첫 줄과 하위 항목이 어울리지 않으면 항목화 강제하지 않음, 각 항목 사이 한 줄 띄움 필수, 하위 항목은 "입니다" 없이 "항목: 내용" 형식.
    - 붙임: "붙임"은 한 번만 앞에 쓰고, 여러 개일 경우 "1. OOO N부."로 번호 붙여 개행, 마지막 항목 뒤 공백 2칸 후 "끝.", 하나면 "붙임  OOO N부.  끝.", 붙임이 없으면 '붙임' 항목 자체 생략.
  ...
  } else if (!hasRelated && contentLines.length === 1) {
    prompt += '  끝.';
  }
